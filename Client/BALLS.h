...
public:
	...
	void pereuchBALLS() {
		system("cls");
		cout << endl << "¬ведите количество товара в шутках" << endl;
		cout << "‘утбольные м€чи:" << endl;
		cout << brandNike << ": ";
		while (true) {
			try {
				cin >> Nike;
	...
	void showBALLS() {
		cout << endl << "‘утбольные м€чи (шт):" << endl;
	...
	void purchaseBALLS() {
		...
		do {
			...
			{
				cout << "Ќет такого варианта!" << endl;
				cin >> choice5;
			}
			send(Connection, (char*)&choice5, sizeof(choice5), 0);
			switch (choice5) {
			case 1:
				cout << endl << "¬ведите количество приобретаемого товара:" << endl;
				while (true) {
					try {
						cin >> number;
	...
	void summary() {
		...
		if (prevNike == 0 && prevAdidas == 0 && prevSelect == 0 && prevKelme == 0) {
			cout << "¬ данный момент вывести рекомендации по оптимизации закупок футбольных м€чей невозможно" << endl;
		}
		else {
			...
			if (brand == 1) {
				cout << "“акже, в категории << ‘утбольные м€чи >> можно увеличить размер закупки м€чей бренда " << brandNike << endl;
			}
			if (brand == 2) {
				cout << "“акже, в категории << ‘утбольные м€чи >> можно увеличить размер закупки м€чей бренда " << brandAdidas << endl;
			}
			if (brand == 3) {
				cout << "“акже, в категории << ‘утбольные м€чи >> можно увеличить размер закупки м€чей бренда " << brandSelect << endl;
			}
			if (brand == 4) {
				cout << "“акже, в категории << ‘утбольные м€чи >> можно увеличить размер закупки м€чей бренда " << brandKelme << endl;
			}
		}
	}
	~BALLS() {};
